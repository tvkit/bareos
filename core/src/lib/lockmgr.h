...
/*
 * Search a deadlock after a fatal signal no lock are granted, so the program
 * must be stopped.
 */
bool LmgrDetectDeadlockUnlocked();

/*
 * This function will run your thread with LmgrInitThread() and
 * LmgrCleanupThread().
 */
 ...
 */
int BthreadKill(pthread_t thread,
                int sig,
                const char *file = "*unknown*",
                int line         = 0);

#define BTHREAD_MUTEX_NO_PRIORITY \
  {                               \
...
#define BTHREAD_MUTEX_PRIORITY(p) \
  {                               \
...
#define BthreadCondTimedwait(x, y, z) \
  BthreadCondTimedwait_p(x, y, z, __FILE__, __LINE__)

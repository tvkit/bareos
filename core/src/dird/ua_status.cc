...
void ListDirStatusHeader(UaContext *ua)
{
  ...
  ua->SendMsg(_("Daemon started %s. Jobs: run=%d, running=%d mode=%d db=%s\n"),
              dt, num_jobs_run, JobCount(), (int)DEVELOPER_MODE,
              dbdrivers.c_str());
      ...
      case slot_type_drive:
        switch (vl1->Content) {
          case slot_content_full:
            ua->SendMsg(slot_api_drive_full_format, 'D', vl1->Slot, vl1->Loaded,
                        vl1->VolName);
            break;
            ...
            if (vl2) {
              switch (vl1->Type) {
                case slot_type_normal:
                  ...
                  break;
                case slot_type_import:
                  ContentSendInfoJson(ua, "import_slot", vl1->Slot,
                                      vl2->VolName);
                  break;
                default:
                  break;
        ...
        switch (vl1->Content) {
          case slot_content_empty:
            if (vl1->Type == slot_type_normal) {
              ...
              vl2 = vol_is_loaded_in_drive(store, vol_list, vl1->Slot);
              if (!vl2) {
                ua->SendMsg(slot_hformat, vl1->Slot, '*', "?", "?", "?", "?");
                continue;
              }
            } else {
              ua->SendMsg(slot_hformat, vl1->Slot, '@', "?", "?", "?", "?");
          ...
          case slot_content_full:
            ...
            if (mr.VolumeName[0] && ua->db->GetMediaRecord(ua->jcr, &mr)) {
              ...
              if (vl1->Type == slot_type_import) {
                ua->SendMsg(slot_hformat, vl1->Slot, '@', mr.VolumeName,

...
JobControlRecord *wait_for_next_job(char *one_shot_job_to_run)
{
  ...
  if (first) {
    ...
    if (one_shot_job_to_run) { /* one shot */
      job =
          (JobResource *)my_config->GetResWithName(R_JOB, one_shot_job_to_run);
      if (!job) {
  ...
  foreach_res (job, R_JOB) {
    ...
    for (run = sched->run; run; run = run->next) {
      ...
      Dmsg7(000, "run h=%d m=%d md=%d wd=%d wom=%d woy=%d yday=%d\n", hour,
            month, mday, wday, wom, woy, yday);
      Dmsg6(000, "bitset bsh=%d bsm=%d bsmd=%d bswd=%d bswom=%d bswoy=%d\n",
            BitIsSet(hour, run->hour), BitIsSet(month, run->month),
            BitIsSet(mday, run->mday), BitIsSet(wday, run->wday),
            BitIsSet(wom, run->wom), BitIsSet(woy, run->woy));
      ...
      Dmsg6(000, "nh_bitset bsh=%d bsm=%d bsmd=%d bswd=%d bswom=%d bswoy=%d\n",
            BitIsSet(nh_hour, run->hour), BitIsSet(nh_month, run->month),
            BitIsSet(nh_mday, run->mday), BitIsSet(nh_wday, run->wday),
            BitIsSet(nh_wom, run->wom), BitIsSet(nh_woy, run->woy));
                    ...
                    time_t runtime)
{
  ...
  if (((runtime - run->last_run) < 61) || ((runtime + 59) < now)) {
    ...
    Dmsg4(000, "Drop: Job=\"%s\" run=%lld. last_run=%lld. now=%lld\n",
          job->hdr.name, (utime_t)runtime, (utime_t)run->last_run,
          (utime_t)now);
    fflush(stdout);
#endif
    return;

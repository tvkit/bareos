                                ...
                                bool label_encrypt, bool yes)
{
   ...
   foreach_dlist(vl, vol_list->contents) {
      if (!vl->VolName || !bit_is_set(vl->LogicalDriveNumber - 1, slot_list)) {
         continue;
      }
      ua->send_msg("%4d  %s\n", vl->LogicalDriveNumber, vl->VolName);
   }
   ...
   foreach_dlist(vl, vol_list->contents) {
      ...
      if (ua->db->get_media_record(ua->jcr, &mr)) {
         if (mr.VolBytes != 0) {
            ua->warning_msg(_("Media record for Slot %hd Volume \"%s\" already exists.\n"),
                            vl->LogicalDriveNumber, mr.VolumeName);
      ...
      mr.Slot = vl->LogicalDriveNumber;
      send_label_request(ua, store, &mr, NULL, &pr, media_record_exists, false, drive, vl->LogicalDriveNumber);
   }

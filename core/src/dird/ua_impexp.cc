        ...
        if (BitIsSet(vl->Slot - 1, slot_list)) {
          switch (content) {
            case slot_content_full:
               ...
               */
              if (vl->Content == content) { continue; }
              /*
              ...
              Dmsg1(100, "Deselecting slot %hd doesn't have wanted content.\n",
                    vl->Slot);
              ua->WarningMsg(
                  _("Deselecting slot %hd doesn't have wanted content.\n"),
                  vl->Slot);
              ClearBit(vl->Slot - 1, slot_list);
              break;
      ...
      case slot_content_full:
        switch (vl->Type) {
          case slot_type_drive:
            if (vl->Loaded == slotnr) { return vl; }
            break;
          default:
            if (vl->Slot == slotnr) { return vl; }
            break;
        }
        break;
      default:
  ...
  foreach_dlist (vl1, vol_list->contents) {
    switch (vl1->Type) {
      ...
      default:
         ...
         */
        if (src_slot_list && !BitIsSet(vl1->Slot - 1, src_slot_list)) {
          continue;
        }
      ...
      case slot_type_drive:
        switch (vl1->Content) {
          case slot_content_full:
            ...
            if (vl2 && vl2->Content == slot_content_full &&
                                               ...
                                               slot_number_t max_slots)
{
  ...
  if (IsNameValid(volumename)) {
    foreach_dlist (vl1, vol_list->contents) {
      ...
      switch (vl1->Type) {
        ...
        case slot_type_import:
           ...
           */
          if (wanted_slot_list && !BitIsSet(vl1->Slot - 1, wanted_slot_list)) {
            continue;
          }
     ...
     */
    if (found) {
      SetBit(vl1->Slot - 1, selected_slot_list);
    } else {
    ...
    slot_content content)
{
  ...
  foreach_dlist (vl, vol_list->contents) {
     ...
     */
    if (vl->Type != type || vl->Content != content) {
     ...
     * then the slot is not really empty.
     */
    if (type == slot_type_normal && content == slot_content_empty &&
        vol_is_loaded_in_drive(store, vol_list, vl->Slot) != NULL) {
      Dmsg3(100,
    ...
    switch (vl->Type) {
      ...
      case slot_type_import:
        if (BitIsSet(vl->Slot - 1, slot_list)) {
           ...
           */
          if (vl->Type == type && vl->Content == content) { continue; }
                                             ...
                                             slot_number_t destination)
{
  ...
  foreach_dlist (vl2, vol_list->contents) {
    switch (vl2->Type) {
      case slot_type_drive:
        continue;
      default:
        if (vl2->Slot == destination) { found = true; }
        break;
    }
    if (found) { break; }
  }
  ...
  if (vl1 && vl2) {
    ...
    Dmsg5(100,
          ...
          vl2->Slot, (vl2->VolName) ? vl2->VolName : "NULL", vl2->Content,
          vl1->Slot, vl1->Content);
    return true;
  }
  return false;
}
                                  ...
                                  changer_vol_list_t *vol_list)
{
  ...
  foreach_dlist (vl1, vol_list->contents) {
     ...
     */
    if (found || vl1->Type != slot_type_drive) { break; }
  }
  ...
  foreach_dlist (vl2, vol_list->contents) {
    switch (vl2->Type) {
      case slot_type_drive:
        continue;
      default:
        if (vl2->Slot == vl1->Loaded) { found = true; }
        break;
    }
    if (found) { break; }
  }
...
static bool PerformMoveOperation(UaContext *ua, enum e_move_op operation)
{
  ...
  switch (operation) {
    case VOLUME_IMPORT:
      ...
      if (nr_enabled_src_slots == 0) {
        ...
        nr_enabled_src_slots =
            auto_fill_slot_selection(store.store, vol_list, src_slot_list,
                                     slot_type_import, slot_content_full);
      } else {
        /*
      ...
      if (nr_enabled_dst_slots == 0) {
        ...
        nr_enabled_dst_slots =
            auto_fill_slot_selection(store.store, vol_list, dst_slot_list,
                                     slot_type_normal, slot_content_empty);
        if (nr_enabled_src_slots > nr_enabled_dst_slots) {
          ua->WarningMsg(
      ...
      visited_slot_list =
          move_volumes_in_autochanger(ua, operation, store.store, vol_list,
    ...
    case VOLUME_EXPORT:
       ...
       */
      if (!verify_slot_list(store.store, vol_list, src_slot_list,
                            slot_type_normal, slot_content_full)) {
        ua->WarningMsg(
      ...
      if (nr_enabled_dst_slots == 0) {
        ...
        nr_enabled_dst_slots =
            auto_fill_slot_selection(store.store, vol_list, dst_slot_list,
                                     slot_type_import, slot_content_empty);
        if (nr_enabled_src_slots > nr_enabled_dst_slots) {

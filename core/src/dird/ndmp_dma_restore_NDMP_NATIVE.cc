...
static bool DoNdmpNativeRestore(JobControlRecord *jcr)
{
  ...
  for (ndmmedia *media = ndmp_sess.control_acb->job.media_tab.head; media;
       media           = media->next) {
  ...
  for (ndmmedia *media = ndmp_sess.control_acb->job.media_tab.head; media;
    ...
    ndmp_slot        = LookupStorageMapping(jcr->res.rstore, slot_type_normal,
                                     LOGICAL_TO_PHYSICAL, media->slot_addr);
   ...
   */
  ndmp_sess.control_acb->job.robot_target =
      (struct ndmscsi_target *)actuallymalloc(sizeof(struct ndmscsi_target));
  if (ndmscsi_target_from_str(ndmp_sess.control_acb->job.robot_target,
                              jcr->res.rstore->ndmp_changer_device) != 0) {

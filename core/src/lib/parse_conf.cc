...
{
  TlsResource *own_tls_resource =
      reinterpret_cast<TlsResource *>(GetNextRes(r_own_, nullptr));
  if (!own_tls_resource) {
...
TlsPolicy ConfigurationParser::GetTlsPolicyForResourceCodeAndName(
    const std::string &r_code_str,
    const std::string &name) const
{
  uint32_t r_code =
      qualified_resource_name_type_converter_->StringToResourceType(r_code_str);
  if (r_code < 0) { return TlsPolicy::kBnetTlsUnknown; }

  TlsResource *foreign_tls_resource =
      reinterpret_cast<TlsResource *>(GetResWithName(r_code, name.c_str()));
...
bool ConfigurationParser::GetConfiguredTlsPolicyFromCleartextHello(
    const std::string &r_code_str,

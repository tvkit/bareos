 ...
 * cvs co
 * :pserver:anonymous@libdbi-drivers.cvs.sourceforge.net:/cvsroot/libdbi-drivers
 */
...
bool BareosDbDBI::OpenDatabase(JobControlRecord *jcr)
{
  ...
  if (numdrivers < 0) {
    Mmsg2(&errmsg,
          _("Unable to locate the DBD drivers to DBI interface in: \n"
  ...
  switch (db_type_) {
    case SQL_TYPE_MYSQL:
      dbi_conn_set_option(db_handle_, "host",
  ...
  if (dbstat != 0) {
    Mmsg3(&errmsg,
          _("Unable to connect to DBI interface. Type=%s Database=%s User=%s\n"
  ...
  if (!CheckTablesVersion(jcr, this)) { goto bail_out; }
...
void BareosDbDBI::CloseDatabase(JobControlRecord *jcr)
{
  ...
  if (ref_count_ == 0) {
    ...
    if (db_driverdir_) { free(db_driverdir_); }
    delete this;
    if (db_list->size() == 0) {
      delete db_list;
      db_list = NULL;
 ...
 */
void BareosDbDBI::UnescapeObject(JobControlRecord *jcr,
                                 char *from,
 ...
 */
bool BareosDbDBI::SqlQueryWithHandler(const char *query,
...
bail_out:
  ...
  // dbi_conn_error(db_handle_, &dbi_errmsg);
  Dmsg4(500,
        "SqlQueryWithoutHandler we failed dbi error: "
        "'%s' '%p' '%d' flag '%d''\n",
...
void BareosDbDBI::SqlFreeResult(void)
{
   ...
   *
   * Using a queue to store all pointer allocate is a good way to free all
 ...
 */
static char *dbi_getvalue(dbi_result *result,
                          int row_number,
                          unsigned int column_number)
{
   ...
   */
  Dmsg3(600,
        "dbi_getvalue pre-starting result '%p' row number '%d' column number "
        "'%d'\n",
  ...
  Dmsg3(
      600,
      "dbi_getvalue starting result '%p' row number '%d' column number '%d'\n",
      result, row_number, column_number);
    ...
    switch (dbitype) {
      ...
      case DBI_TYPE_DATETIME:
        ...
        if (last == -1) {
          ...
          Blocaltime(&last, &tm);
          field_length = Bsnprintf(
              buf, 20, "%04d-%02d-%02d %02d:%02d:%02d", (tm.tm_year + 1900),
...
SQL_ROW BareosDbDBI::SqlFetchRow(void)
{
  ...
  if (row_number_ <= num_rows_ && row_number_ != DBI_ERROR_BADPTR) {
    ...
    for (j = 0; j < num_fields_; j++) {
      ...
      Dmsg4(500,
            "SqlFetchRow row[%d] field: '%p' in queue: '%p' has value: '%s'\n",
...
uint64_t BareosDbDBI::SqlInsertAutokeyRecord(const char *query,
  ...
  if (!fields_ || fields_size_ < num_fields_) {
    ...
    for (i = 0; i < num_fields_; i++) {
      ...
      for (j = 0; j < num_rows_; j++) {
        ...
        if (MaxLength < this_length) { MaxLength = this_length; }
      }
      ...
      Dmsg4(500,
            ...
            "IsNull=%d\n",
            fields_[i].name, fields_[i].MaxLength, fields_[i].type,
            fields_[i].flags);
    }
  }
...
bool BareosDbDBI::SqlBatchEnd(JobControlRecord *jcr, const char *error)
{
  ...
  switch (db_type_) {
    case SQL_TYPE_MYSQL:
      ...
      break;
    case SQL_TYPE_POSTGRESQL:
      custom_function = (custom_function_end_t)dbi_driver_specific_function(
          dbi_conn_get_driver(myconn), "PQputCopyEnd");
      ...
      if (res <= 0) {
        Dmsg0(500, "we failed\n");
        status_ = (dbi_error_flag)0;
        // Mmsg1(&errmsg, _("error ending batch mode: %s"),
...
#endif
{
  ...
  if (strlen(db_driver) < 5 || db_driver[3] != ':' ||
      !bstrncasecmp(db_driver, "dbi", 3)) {

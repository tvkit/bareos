 ...
 */
static void ObjectStoreLogfunc(dpl_ctx_t *ctx,
...
int object_store_device::d_open(const char *pathname, int flags, int mode)
{
...
#if 1
  Mmsg1(errmsg,
        _("Object Storage devices are not yet supported, please disable %s\n"),
        dev_name);
  return -1;
#endif
  ...
  if (!ctx_) {
    ...
    switch (status) {
      ...
      default:
        Mmsg2(errmsg,
              _("Failed to login for voume %s using dpl_login(): ERR=%s.\n"),
     ...
     */
    if (object_bucketname_) { ctx_->cur_bucket = object_bucketname_; }
  }
...
/**
 * Open a directory on the object store and find out size information for a
 * file.
 */
static inline size_t ObjectStoreGetFileSize(dpl_ctx_t *ctx,
                                            const char *filename)
{
  void *dir_hdl;
  dpl_status_t status;
...
boffset_t object_store_device::d_lseek(DeviceControlRecord *dcr,
                                       boffset_t offset,
                                       int whence)
{
  switch (whence) {
...
{
  /*
   * libdroplet doesn't have a truncate function so unlink the volume and create
   * a new empty one.
   */
  if (vfd_) {
    dpl_status_t status;
...
object_store_device::~object_store_device()
{
  ...
  if (object_configstring_) { free(object_configstring_); }

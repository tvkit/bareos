 ...
 *  - password
 */
static inline bool IsSameStorageDaemon(StorageResource *rstore,
                                       StorageResource *wstore)
{
                              ...
                              const char *type)
{
  ...
  if (!jcr->db->SqlQuery(query.c_str(), UniqueNameHandler,
                         (void *)item_chain)) {
...
bool DoMigrationInit(JobControlRecord *jcr)
{
  ...
  if (jcr->MigrateJobId != 0) {
    ...
    jcr->remote_replicate =
        !IsSameStorageDaemon(jcr->res.rstore, jcr->res.wstore);
...
static inline bool DoActualMigration(JobControlRecord *jcr)
{
  ...
  if (jcr->remote_replicate) {
    ...
    if (!StartStorageDaemonJob(mig_jcr, NULL, mig_jcr->res.wstorage,
                               /* send_bsr */ false)) {
      goto bail_out;
    }
     ...
     */
    if (!StartStorageDaemonJob(jcr, jcr->res.rstorage, jcr->res.wstorage,
                               /* send_bsr */ true)) {
      FreePairedStorage(jcr);
      return false;
    }
  ...
  if (jcr->remote_replicate) {
     ...
     */
    if (wstore->SDDport == 0) { wstore->SDDport = wstore->SDport; }

    /*
     * TLS Requirement
     */
    tls_need = wstore->IsTlsConfigured() ? TlsPolicy::kBnetTlsAuto
    ...
    Mmsg(command, replicatecmd, mig_jcr->JobId, mig_jcr->Job,
         connection_target_address, wstore->SDDport, tls_need,
         mig_jcr->sd_auth_key);

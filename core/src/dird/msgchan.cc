...
bool UpdateDeviceRes(JobControlRecord *jcr, DeviceResource *dev)
{
  PoolMem device_name;
  BareosSocket *sd;
  if (!ConnectToStorageDaemon(jcr, 5, 30, false)) { return false; }
  sd = jcr->store_bsock;
  ...
  if (BgetDirmsg(sd) <= 0) { return false; }
  return true;
}
#endif
                           ...
                           bool send_bsr)
{
  ...
  sd->fsend(jobcmd, edit_int64(jcr->JobId, ed1), jcr->Job, job_name.c_str(),
            ...
            jcr->res.job->PreferMountedVolumes,
            edit_int64(jcr->spool_size, ed2), jcr->rerunning, jcr->VolSessionId,
            jcr->VolSessionTime, remainingquota, jcr->getJobProtocol(),
            backup_format.c_str());
...
bool StartStorageDaemonMessageThread(JobControlRecord *jcr)
{
  ...
  while (!jcr->SD_msg_chan_started) {
    Bmicrosleep(0, 50);
    if (JobCanceled(jcr) || jcr->sd_msg_thread_done) { return false; }
  }
...
extern "C" void *msg_thread(void *arg)
{
  ...
  if (IsBnetError(sd)) { jcr->SDJobStatus = JS_ErrorTerminated; }

...
void CopyRstorage(JobControlRecord *jcr, alist *storage, const char *where)
{
  if (storage) {
    ...
    if (jcr->res.rstorage) { delete jcr->res.rstorage; }
...
void SetRstorage(JobControlRecord *jcr, UnifiedStorageResource *store)
{
  ...
  if (jcr->res.rstorage) { FreeRstorage(jcr); }
  if (!jcr->res.rstorage) {
    jcr->res.rstorage = New(alist(10, not_owned_by_alist));
  }
...
void CopyWstorage(JobControlRecord *jcr, alist *storage, const char *where)
{
  if (storage) {
    ...
    if (jcr->res.wstorage) { delete jcr->res.wstorage; }
...
void SetWstorage(JobControlRecord *jcr, UnifiedStorageResource *store)
{
  ...
  if (!store->store) { return; }
  if (jcr->res.wstorage) { FreeWstorage(jcr); }
  if (!jcr->res.wstorage) {
    jcr->res.wstorage = New(alist(10, not_owned_by_alist));
  }
    ...
    case JT_RESTORE:
      ...
      if (jcr->res.rstorage) {
        ...
        foreach_alist (pstore, jcr->res.rstorage) {
          ...
          while (store) {
            if (store->paired_storage == pstore) { break; }
          ...
          if (store) {
             ...
             */
            if (pstore == jcr->res.rstore) { jcr->res.pstore = store; }
          }
        }
      } else {
        Jmsg(jcr, M_FATAL, 0,
 ...
 */
slot_number_t LookupStorageMapping(StorageResource *store,
                                   slot_type slot_type,
                                   s_mapping_type type,
                                   slot_number_t slot)
{
  ...
  if (store->rss->storage_mappings) {
    ...
    while (mapping) {
      ...
      mapping =
          (storage_mapping_t *)store->rss->storage_mappings->next(mapping);
    }
  }

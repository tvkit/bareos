                                               ...
                                               void *arg))
{
  ...
  if ((status = pthread_attr_init(&wq->attr)) != 0) { return status; }
  if ((status = pthread_attr_setdetachstate(&wq->attr,
...
int WorkqDestroy(workq_t *wq)
{
  ...
  if (wq->valid != WORKQ_VALID) { return 0; /* nothing to do */ }
  P(wq->mutex);
             ...
             workq_ele_t **work_item)
{
  ...
  if (wq->num_workers < wq->max_workers) {
    ...
    if ((status = pthread_create(&id, &wq->attr, workq_server, (void *)wq)) !=
        0) {
      V(wq->mutex);
      return status;
    }

                                 ...
                                 int stream)
{
   ...
   * Sanity check
   */
  if (xattr_data->u.build->content_length <= 0) { return bxattr_exit_ok; }
 ...
 */
static bxattr_exit_code (*os_build_xattr_streams)(JobControlRecord *jcr,
                                                  xattr_data_t *xattr_data,
                                                  FindFilesPacket *ff_pkt) =
    aix_build_xattr_streams;
static bxattr_exit_code (*os_parse_xattr_streams)(JobControlRecord *jcr,
                                                  xattr_data_t *xattr_data,
                                                  int stream,
                                                  char *content,
                                                  uint32_t content_length) =
                                           ...
                                           int stream)
{
  ...
  switch (st.st_mode & S_IFMT) {
    ...
    case S_IFLNK:
      ...
      if (retval == bxattr_exit_ok) { xattr_data->u.build->nr_saved++; }
  ...
  if (xattr_data->u.build->nr_saved == 0) {
    ...
    if (retval != bxattr_exit_ok) { goto bail_out; }

...
class lmgr_node_t : public SmartAlloc {
 public:
  ...
  lmgr_node_t(void *n, void *c) { init(n, c); }
  ...
  void MarkAsSeen(lmgr_color_t c) { seen = c; }
...
class lmgr_lock_t : public SmartAlloc {
 public:
  ...
  void set_granted() { state = LMGR_LOCK_GRANTED; }
...
static void SearchAllNode(dlist *g, lmgr_node_t *v, alist *ret)
{
  lmgr_node_t *n;
  foreach_dlist (n, g) {
    if (v->child == n->node) { ret->append(n); }
  }
}
...
class lmgr_thread_t : public SmartAlloc {
 public:
  ...
  void _dump(FILE *fp)
  {
    ...
    for (int i = 0; i <= current; i++) {
      fprintf(fp, "   lock=%p state=%s priority=%i %s:%i\n", lock_list[i].lock,
              ...
              lock_list[i].priority, lock_list[i].file, lock_list[i].line);
    }
  }
  ...
  {
    for (int j = i + 1; j <= current; j++) { lock_list[i] = lock_list[j]; }
    if (current >= 0) {
    ...
    Lmgr_p(&mutex);
    {
      if (lock_list[current].lock == m) {
        ...
        FPmsg4(0, "ERROR: wrong P/V order pos=%i lock=%p %s:%i\n", current,
               lock_list[current].lock, lock_list[current].file,
               lock_list[current].line);
        for (int i = current - 1; i >= 0; i--) { /* already seen current */
          FPmsg4(0, "ERROR: wrong P/V order pos=%i lock=%p %s:%i\n", i,
                 lock_list[i].lock, lock_list[i].file, lock_list[i].line);
...
bool LmgrDetectDeadlockUnlocked()
{
  ...
  ret = ContainsCycle(g);
  if (ret) { printf("Found a deadlock !!!!\n"); }
...
void cln_hdl(void *a) { LmgrCleanupThread(); }
...
void LmgrCleanupMain()
{
  ...
  if (!global_mgr) { return; }
  if (use_undertaker) { pthread_cancel(undertaker); }
  LmgrCleanupThread();
  Lmgr_p(&lmgr_global_mutex);
  {
    temp       = global_mgr;
 ...
 */
int BthreadKill(pthread_t thread, int sig, const char *file, int line)
{
...
int LmgrMutexIsLocked(void *m)
{
  ...
  for (int i = 0; i <= self->current; i++) {
    if (self->lock_list[i].lock == m) { return 1; /* locked by us */ }
  }
...
extern "C" void *lmgr_thread_launcher(void *x)
{
  void *ret = NULL;
  LmgrInitThread();
  pthread_cleanup_push(cln_hdl, NULL);
 ...
 */
void DbgPrintLock(FILE *fp) { FPmsg0(000, "lockmgr disabled\n"); }
...
int main(int argc, char **argv)
{
  ...
  for (int j = 0; j < 200; j++) { pthread_create(&tab[j], NULL, thx, NULL); }
  for (int j = 0; j < 200; j++) {

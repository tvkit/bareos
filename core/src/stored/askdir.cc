 ...
 */
bool StorageDaemonDeviceControlRecord::dir_update_device(JobControlRecord *jcr,
                                                         Device *dev)
{
  ...
  ok = dir->fsend(Device_update, jcr->Job, dev_name.c_str(),
                  dev->CanAppend() != 0, dev->CanRead() != 0, dev->num_writers,
                  dev->IsOpen() != 0, dev->IsLabeled() != 0,
                  dev->IsOffline() != 0, dev->reserved_device,
                  dev->IsTape() ? 100000 : 1, dev->autoselect, 0,
...
bool StorageDaemonDeviceControlRecord::dir_update_changer(JobControlRecord *jcr,
                                                          AUTOCHANGER *changer)
{
  BareosSocket *dir = jcr->dir_bsock;
  ...
  ok = dir->fsend(Device_update, jcr->Job, dev_name.c_str(), /* Changer name */

...
int elasto_device::d_open(const char *pathname, int flags, int mode)
{
  ...
  if (min_block_size == 0 || (min_block_size % ELASTO_GRANULARITY) != 0) {
    Emsg2(M_ABORT, 0,
          _("Device minimum block size (%d) not multiple of elasto blocksize "
            "size (%d)\n"),
  ...
  if (max_block_size == 0 || (max_block_size % ELASTO_GRANULARITY) != 0) {
    Emsg2(M_ABORT, 0,
          _("Device maximum block size (%d) not multiple of elasto blocksize "
            "size (%d)\n"),
  ...
  if (flags & O_CREAT) { elasto_flags = ELASTO_FOPEN_CREATE; }
...
int elasto_device::d_ioctl(int fd, ioctl_req_t request, char *op) { return -1; }

boffset_t elasto_device::d_lseek(DeviceControlRecord *dcr,
                                 boffset_t offset,
                                 int whence)
{
  switch (whence) {
...
elasto_device::~elasto_device()
{
  ...
  if (elasto_configstring_) { free(elasto_configstring_); }

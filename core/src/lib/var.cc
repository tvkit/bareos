 ...
 */
static int var_mvxprintf(int (*output)(void *ctx,
                                       const char *buffer,
                                       int bufsize),
                         void *ctx,
                         const char *format,
  ...
  while (*format != '\0') {
    if (*format == '%') {
      ...
      if (c == '%') {
        ...
        /* expand "%s" */
        if ((cp = (char *)va_arg(ap, char *)) == NULL) cp = "(null)";
        n = strlen(cp);
      } else if (c == 'd') {
        /* expand "%d" */
        d = (int)va_arg(ap, int);
      ...
      /* plain text */
      cp = (char *)format;
      if ((format = strchr(cp, '%')) == NULL) format = strchr(cp, '\0');
      n = format - cp;
    }
    /* perform output operation */
    if (output != NULL)
      if ((n = output(ctx, cp, n)) == -1) break;
    bytes += n;
  }
  return bytes;
}
...
static int var_mvsnprintf_cb(void *_ctx, const char *buffer, int bufsize)
{
  var_mvsnprintf_cb_t *ctx = (var_mvsnprintf_cb_t *)_ctx;

  if (bufsize > ctx->buflen) return -1;
...
static int var_mvsnprintf(char *buffer,
                          int bufsize,
                          const char *format,
                          va_list ap)
{
  ...
  if (format == NULL) return -1;
  if (buffer != NULL && bufsize == 0) return -1;
  if (buffer == NULL) /* just determine output length */
...
static var_parse_t *var_parse_push(var_parse_t *lower, var_parse_t *upper)
{
...
static var_parse_t *var_parse_pop(var_parse_t *upper)
{
  if (upper == NULL) return NULL;
  return upper->lower;
}
...
static int tokenbuf_isundef(tokenbuf_t *buf)
{
  if (buf->begin == NULL && buf->end == NULL) return 1;
  return 0;
}

static int tokenbuf_isempty(tokenbuf_t *buf)
{
  if (buf->begin == buf->end) return 1;
  return 0;
}

static void tokenbuf_set(tokenbuf_t *buf,
                         const char *begin,
                         const char *end,
                         int buffer_size)
{
...
static int tokenbuf_assign(tokenbuf_t *buf, const char *data, int len)
{
  char *p;

  if ((p = (char *)malloc(len + 1)) == NULL) return 0;
  memcpy(p, data, len);
  buf->begin            = p;
  buf->end              = p + len;
...
static int tokenbuf_append(tokenbuf_t *output, const char *data, int len)
{
  char *new_buffer;
  int new_size;
  char *tmp;
  ...
  if (len > 0) memcpy((char *)output->end, data, len);
  output->end += len;
  *((char *)output->end) = EOS;
  return 1;
}
...
{
  if (buf->begin != NULL && buf->buffer_size > 0) free((char *)buf->begin);
...
static void expand_range(char a, char b, char_class_t chrclass)
{
  do {
    chrclass[(int)a] = 1;
  } while (++a <= b);
...
static var_rc_t expand_character_class(const char *desc, char_class_t chrclass)
{
  int i;

  /* clear the class array. */
  for (i = 0; i < 256; ++i) chrclass[i] = 0;

  /* walk through class description and set appropriate entries in array */
  while (*desc != EOS) {
    if (desc[1] == '-' && desc[2] != EOS) {
      if (desc[0] > desc[2]) return VAR_ERR_INCORRECT_CLASS_SPEC;
...
static var_rc_t expand_octal(const char **src, char **dst, const char *end)
{
  ...
  c = **src - '0';
  if (c > 3) return VAR_ERR_OCTAL_TOO_LARGE;
  c *= 8;
  (*src)++;
...
static var_rc_t expand_simple_hex(const char **src, char **dst, const char *end)
{
  ...
  if (end - *src < 2) return VAR_ERR_INCOMPLETE_HEX;
  if (!expand_ishex(**src) || !expand_ishex((*src)[1]))
...
static var_rc_t expand_grouped_hex(const char **src,
                                   char **dst,
                                   const char *end)
{
  var_rc_t rc;
  ...
  while (*src < end && **src != '}') {
    if ((rc = expand_simple_hex(src, dst, end)) != VAR_OK) return rc;
    (*src)++;
  }
  if (*src == end) return VAR_ERR_INCOMPLETE_GROUPED_HEX;
...
{
  if (*src == end) return VAR_ERR_INCOMPLETE_HEX;
  if (**src == '{') {
    (*src)++;
...
static int ParseVariable(var_t *var,
                         var_parse_t *ctx,
                         const char *begin,
                         const char *end,
                         tokenbuf_t *result);
static int parse_numexp(var_t *var,
                        var_parse_t *ctx,
                        const char *begin,
                        const char *end,
                        int *result,
...
static int parse_opargtext_or_variable(var_t *var,
                                       var_parse_t *ctx,
                                       const char *begin,
                                       const char *end,
                                       tokenbuf_t *result)
...
static int parse_exptext_or_variable(var_t *var,
                                     var_parse_t *ctx,
                                     const char *begin,
                                     const char *end,
                                     tokenbuf_t *result)
...
static int parse_substext_or_variable(var_t *var,
                                      var_parse_t *ctx,
                                      const char *begin,
                                      const char *end,
                                      tokenbuf_t *result)
                                   ...
                                   tokenbuf_t *dst)
{
  ...
  while (p != src->end) {
    if ((src->end - p) >= 3 && p[1] == '-') {
      if (*p > p[2]) return VAR_ERR_INCORRECT_TRANSPOSE_CLASS_SPEC;
      for (c = *p, d = p[2]; c <= d; ++c) {
        if (!tokenbuf_append(dst, (char *)&c, 1)) return VAR_ERR_OUT_OF_MEMORY;
      }
      p += 3;
    } else {
      if (!tokenbuf_append(dst, p, 1)) return VAR_ERR_OUT_OF_MEMORY;
      p++;
    }
  }
  return VAR_OK;
...
/* operation: transpose */
static int op_transpose(var_t *var,
                        var_parse_t *ctx,
                        tokenbuf_t *data,
                        tokenbuf_t *search,
                        tokenbuf_t *replace)
...
static int op_search_and_replace(var_t *var,
                                 var_parse_t *ctx,
                                 tokenbuf_t *data,
                                 tokenbuf_t *search,
                                 tokenbuf_t *replace,
                                 tokenbuf_t *flags)
{
  ...
  if (search->begin == search->end) return VAR_ERR_EMPTY_SEARCH_STRING;
  ...
  if (no_regex) {
    ...
    for (p = mydata.begin; p < mydata.end;) {
      ...
      rc = regexec(&preg, p, sizeof(pmatch) / sizeof(regmatch_t), pmatch,
                   regexec_flag);
      if (rc != 0) {
        /* no (more) matching */
                     ...
                     int isrange)
{
  ...
  if ((data->end - data->begin) < num1) return VAR_ERR_OFFSET_OUT_OF_BOUNDS;
  p = data->begin + num1;

  /* if num2 is zero, we copy the rest from there. */
  if (num2 == 0) {
    if (!tokenbuf_assign(&res, p, data->end - p)) return VAR_ERR_OUT_OF_MEMORY;
  } else {
    /* ok, then use num2. */
    if (isrange) {
      if ((p + num2) > data->end) return VAR_ERR_RANGE_OUT_OF_BOUNDS;
      if (!tokenbuf_assign(&res, p, num2)) return VAR_ERR_OUT_OF_MEMORY;
    } else {
      if (num2 < num1) return VAR_ERR_OFFSET_LOGIC;
      if ((data->begin + num2) > data->end) return VAR_ERR_RANGE_OUT_OF_BOUNDS;
                      ...
                      char position)
{
  ...
  if (fill->begin == fill->end) return VAR_ERR_EMPTY_PADDING_FILL_STRING;
  tokenbuf_init(&result);
  if (position == 'l') {
    /* left padding */
    ...
    if (i > 0) {
      ...
      if (!tokenbuf_append(data, fill->begin, i)) return VAR_ERR_OUT_OF_MEMORY;
    }
  } else if (position == 'r') {
    /* right padding */
    i = width - (data->end - data->begin);
...
static int parse_operation(var_t *var,
                           var_parse_t *ctx,
                           const char *begin,
                           const char *end,
                           tokenbuf_t *data)
{
  ...
  switch (tolower(*p)) {
    ...
    case 's': {
      /* search and replace. */
      p++;
      if (*p != '/') return VAR_ERR_MALFORMATTED_REPLACE;
      p++;
    ...
    case 'p': {
      /* padding. */
      p++;
      if (*p != '/') return VAR_ERR_MALFORMATTED_PADDING;
      p++;
      rc = parse_integer(var, ctx, p, end, &num1);
      if (rc == 0) {
    ...
    case '%': {
      ...
      if (data->begin != NULL && var->cb_operation_fct != NULL) {
        ...
        rc = (*var->cb_operation_fct)(var, var->cb_operation_ctx, op_ptr,
                                      op_len, arg_ptr, arg_len, val_ptr,
                                      val_len, &out_ptr, &out_len, &out_size);
        if (rc < 0) {
          if (arg_ptr != NULL) free((void *)arg_ptr);
          goto error_return;
        }
        tokenbuf_free(data);
...
static int parse_numexp_operand(var_t *var,
                                var_parse_t *ctx,
                                const char *begin,
                                const char *end,
                                int *result,
                                int *failed)
{
  ...
  p = begin;
  tokenbuf_init(&tmp);
  if (p == end) return VAR_ERR_INCOMPLETE_INDEX_SPEC;
  ...
  if (*p == '(') {
    ...
    if (*p != ')') return VAR_ERR_UNCLOSED_BRACKET_IN_INDEX;
    p++;
  }
  ...
  else if (var->syntax.index_mark != EOS && *p == var->syntax.index_mark) {
    p++;
    *result = ctx->index_this;
    if (ctx->rel_lookup_flag) ctx->rel_lookup_cnt++;
  }
...
static int lookup_value(var_t *var,
                        var_parse_t *ctx,
                        const char *var_ptr,
                        int var_len,
                        int var_inc,
                        ...
                        int *val_size)
{
  ...
  rc = (*var->cb_value_fct)(var, var->cb_value_ctx, var_ptr, var_len, var_inc,
                            var_idx, val_ptr, val_len, val_size);
...
static int parse_variable_complex(var_t *var,
                                  var_parse_t *ctx,
                                  const char *begin,
                                  const char *end,
                                  tokenbuf_t *result)
{
  ...
  if (p == end || *p != var->syntax.delim_open) return 0;
  p++;
  ...
  if (var->syntax.index_open != EOS && *p == var->syntax.index_open) {
    p++;
  ...
  if (failed) {
    ...
    rc = lookup_value(var, ctx, name.begin, name.end - name.begin, inc, idx,
                         ...
                         tokenbuf_t *result)
{
  ...
  if (p == end || *p != var->syntax.delim_init) return 0;
  p++;
  if (p == end) return VAR_ERR_INCOMPLETE_VARIABLE_SPEC;
...
static var_rc_t parse_looplimits(var_t *var,
                                 var_parse_t *ctx,
                                 const char *begin,
                                 const char *end,
                                 int *start,
                                 ...
                                 int *open_stop)
{
  ...
  if (p == end) return VAR_OK;

  /* parse start delimiter */
  if (*p != var->syntax.delim_open) return VAR_OK;
  p++;
  ...
  if (rc == VAR_ERR_INVALID_CHAR_IN_INDEX_SPEC)
    *start = 0; /* use default */
    ...
    p += rc;
  if (failed) return VAR_ERR_UNDEFINED_VARIABLE;

  /* parse separator */
  if (*p != ',') return VAR_ERR_INVALID_CHAR_IN_LOOP_LIMITS;
  p++;
  ...
  /* parse end delimiter */
  if (*p != var->syntax.delim_close) return VAR_ERR_INVALID_CHAR_IN_LOOP_LIMITS;
  p++;
...
static var_rc_t parse_input(var_t *var,
                            var_parse_t *ctx,
                            const char *begin,
                            const char *end,
                            tokenbuf_t *output,
  ...
  do {
    ...
    re_loop:
      for (i = start; ((open_stop && (loop_limit_length < 0 ||
        ...
        rc = parse_input(var, ctx, p, end, output, recursion_level + 1);
        ...
        if (loop_limit_length < 0) {
          rc2 = parse_looplimits(var, ctx, p + rc + 1, end, &start, &step,
                                 &stop, &open_stop);
          if (rc2 < 0)
            goto error_return;
          else if (rc2 == 0)
      ...
      if (open_stop) output->end = (output->begin + output_backup);
...
var_rc_t var_create(var_t **pvar)
{
  var_t *var;

  if (pvar == NULL) return VAR_RC(VAR_ERR_INVALID_ARGUMENT);
...
{
  if (var == NULL) return VAR_RC(VAR_ERR_INVALID_ARGUMENT);
  free(var);
  return VAR_OK;
}
                      ...
                      int all)
{
  ...
  while (src < end) {
    if (*src == '\\') {
      if (++src == end) return VAR_RC(VAR_ERR_INCOMPLETE_NAMED_CHARACTER);
      switch (*src) {
        case '\\':
          if (!all) { *dst++ = '\\'; }
          *dst++ = '\\';
        ...
        case 'x':
          ++src;
          if ((rc = expand_hex(&src, &dst, end)) != VAR_OK) return VAR_RC(rc);
          break;
        case '0':
        case '1':
        case '2':
                    ...
                    int force_expand)
{
  ...
  if (rc >= 0) {
    ...
    *dst_ptr = (char *)output.begin;
    if (dst_len != NULL) *dst_len = (output.end - output.begin);
    rc = VAR_OK;
  } else {
    /* provide result */
    if (dst_len != NULL) *dst_len = (output.end - output.begin);

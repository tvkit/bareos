...
int BgetDirmsg(BareosSocket *bs, bool allow_any_message)
{
  ...
  for (; !bs->IsStop() && !bs->IsTimedOut();) {
    ...
    if (bs->msg[0] = 'D') { /* Device update */
      ...
      if (sscanf(bs->msg, Device_update, &Job, dev_name.c_str(), &dev_append,
                 &dev_read, &dev_num_writers, &dev_open, &dev_labeled,
                 &dev_offline, &dev_reserved, &dev_max_writers, &dev_autoselect,
                 &dev_autochanger, changer_name.c_str(), media_type.c_str(),
                 volume_name.c_str(), &dev_read_time, &dev_write_time,
                 &dev_read_bytes, &dev_write_bytes) != 19) {
        ...
        bstrncpy(dev->ChangerName, changer_name.c_str(),
                 sizeof(dev->ChangerName));

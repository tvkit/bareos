 ...
 * we map the ndm_media fields into the MediaDbRecord and JobMediaDbRecord
 fields as follows
 *
 ***********************************************************************************************
 *     NDM_MEDIA                        |        MediaDbRecord /
 JobMediaDbRecord                      *
 ***********************************************************************************************
 char     label[NDMMEDIA_LABEL_MAX+1];  |  MediaDbRecord:      char
 VolumeName[MAX_NAME_LENGTH]; unsigned slot_addr;                    |
 MediaDbRecord:      int32_t Slot;
 -----------------------------------------------------------------------------------------------
 unsigned file_mark_offset;             |  JobMediaDbRecord:   uint32_t
 StartBlock; uint64_t n_bytes;                      |  JobMediaDbRecord:
 uint64_t JobBytes;
 ***********************************************************************************************

 Notes:
   * Slot needs to be translated from bareos slotnr to NDMP slot number  and
 back

...
static tapealert_mapping tapealert_mappings[] = {
    ...
    {0x1E,
     (const char
          *)"Hardware A: Tape drive has a problem not read/write related"},
    {0x1F,
     (const char
          *)"Hardware B: Tape drive has a problem not read/write related"},
    {0x20,
     (const char
          *)"Problem with SCSI interface between tape drive and initiator"},
    {0x21,
    ...
    {0x29,
     (const char
          *)"Loader Stray Tape: Stray tape left in drive from prior error"},
...
bool GetTapealertFlags(int fd, const char *device_name, uint64_t *flags)
{
  ...
  *flags       = 0;
   ...
   */
  if (!RecvScsiCmdPage(fd, device_name, (void *)&cdb, cdb_len,

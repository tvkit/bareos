...
static bool AcquireResources(JobControlRecord *jcr)
{
   ...
   */
  if (jcr->res.rstore &&
      jcr->res.rstore == jcr->res.wstore) { /* possible deadlock */
    Jmsg(jcr, M_FATAL, 0,
         _("Job canceled. Attempt to read and write same device.\n"
           "    Read storage \"%s\" (From %s) -- Write storage \"%s\" (From "
           "%s)\n"),
...
bool IncReadStore(JobControlRecord *jcr)
{
  ...
  P(mutex);
  if (jcr->res.rstore->rss->NumConcurrentJobs <
      jcr->res.rstore->MaxConcurrentJobs) {
...
static bool IncWriteStore(JobControlRecord *jcr)
{
  ...
  P(mutex);
  if (jcr->res.wstore->rss->NumConcurrentJobs <
      jcr->res.wstore->MaxConcurrentJobs) {

...
static ResourceItem cli_items[] = {
    ...
    {"FdPort", CFG_TYPE_ADDRESSES_PORT, ITEM(res_client.FDaddrs), 0, CFG_ITEM_DEFAULT, FD_DEFAULT_PORT, NULL,
     NULL},
    {"FdAddress", CFG_TYPE_ADDRESSES_ADDRESS, ITEM(res_client.FDaddrs), 0, CFG_ITEM_DEFAULT, FD_DEFAULT_PORT,
     NULL, NULL},
    {"FdAddresses", CFG_TYPE_ADDRESSES, ITEM(res_client.FDaddrs), 0, CFG_ITEM_DEFAULT, FD_DEFAULT_PORT, NULL,
     NULL},
    ...
    {"SubSysDirectory", CFG_TYPE_DIR, ITEM(res_client.subsys_directory), 0, CFG_ITEM_DEPRECATED, NULL, NULL,
     NULL},
    ...
    {"MaximumConcurrentJobs", CFG_TYPE_PINT32, ITEM(res_client.MaxConcurrentJobs), 0, CFG_ITEM_DEFAULT, "20",
     NULL, NULL},
    {"MaximumConnections", CFG_TYPE_PINT32, ITEM(res_client.MaxConnections), 0, CFG_ITEM_DEFAULT, "42",
     "15.2.3-", NULL},
    {"Messages", CFG_TYPE_RES, ITEM(res_client.messages), R_MSGS, 0, NULL, NULL, NULL},
    {"SdConnectTimeout", CFG_TYPE_TIME, ITEM(res_client.SDConnectTimeout), 0, CFG_ITEM_DEFAULT,
     "1800" /* 30 minutes */, NULL, NULL},
    {"HeartbeatInterval", CFG_TYPE_TIME, ITEM(res_client.heartbeat_interval), 0, CFG_ITEM_DEFAULT, "0", NULL,
     NULL},
    ...
    {"AlwaysUseLmdb", CFG_TYPE_BOOL, ITEM(res_client.always_use_lmdb), 0, CFG_ITEM_DEFAULT, "false", NULL,
     NULL},
...
static void StoreCipher(LEX *lc, ResourceItem *item, int index, int pass)
{
  ...
  if (i != 0) { scan_err1(lc, _("Expected a Crypto Cipher option, got: %s"), lc->str); }
  ScanToEol(lc);
...
static void FreeResource(CommonResourceHeader *sres, int type)
{
  ...
  switch (type) {
    ...
    case R_CLIENT:
      ...
      if (res->res_client.pid_directory) { free(res->res_client.pid_directory); }
      if (res->res_client.subsys_directory) { free(res->res_client.subsys_directory); }
      if (res->res_client.scripts_directory) { free(res->res_client.scripts_directory); }
      if (res->res_client.plugin_directory) { free(res->res_client.plugin_directory); }
      ...
      if (res->res_client.FDsrc_addr) { FreeAddresses(res->res_client.FDsrc_addr); }
      if (res->res_client.pki_keypair_file) { free(res->res_client.pki_keypair_file); }
      ...
      if (res->res_client.allowed_script_dirs) { delete res->res_client.allowed_script_dirs; }

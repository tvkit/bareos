...
#else
#include <varargs.h>
#define VARARGS(func, arg, type) func(va_alist) va_dcl
#define VASTART(ap, name, type) \
  {                             \
    type name;                  \
    va_start(ap);               \
...
{
  syslog(LOG_ERR,
         "usage: smtp [-c cc] [-e errors-to] [-f from] [-m mailhost] [-M] [-r "
         "reply-to] [-s subject] [-v] recipents..\n");
  exit(1);
}
...
char **argv;
{
  ...
  for (i = optind; i < argc; i++) chat("RCPT TO: <%s>\r\n", argv[i]);
  if (cc_addr) chat("RCPT TO: <%s>\r\n", cc_addr);
  ...
  if (subject) fprintf(sfp, "Subject: %s\r\n", subject);

  if (err_addr) fprintf(sfp, "Errors-To: %s\r\n", err_addr);

  if (reply_addr) fprintf(sfp, "Reply-To: %s\r\n", reply_addr);
  ...
  for (i = optind + 1; i < argc; i++) fprintf(sfp, ", %s", argv[i]);
  fprintf(sfp, "\r\n");
...
void toqp(infile, outfile) FILE *infile, *outfile;
{

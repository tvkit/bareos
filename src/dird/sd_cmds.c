...
static char readlabelcmd[] =
   "readlabel %s LogicalSlotNumber=%hd drive=%hd\n";
...
static char readlabelresponse[] =
   "3001 Volume=%s LogicalSlotNumber=%hd";
 ...
 */
char *get_volume_name_from_SD(UAContext *ua, slot_number_t LogicalSlotNumber, drive_number_t drive)
{
   BSOCK *sd;
   STORERES *store = ua->jcr->res.wstore;
    ...
    */
   sd->fsend(readlabelcmd, dev_name, LogicalSlotNumber, drive);
   Dmsg1(100, "Sent: %s", sd->msg);

   /*
    * Get Volume name in this LogicalSlotNumber
    */
   while (sd->recv() >= 0) {
 ...
 *
 * 0:vol2                LogicalSlotNumber num:Volume Name
 *
 * Input (output of mxt-changer listall):
 *
 * Drive content:         D:Drive num:F:LogicalSlotNumber loaded:Volume Name
 * D:0:F:2:vol2        or D:Drive num:E
 * D:1:F:42:vol42
 * D:3:E
 *
 * LogicalSlotNumber content:
 * S:1:F:vol1             S:LogicalSlotNumber num:F:Volume Name
 * S:2:E               or S:LogicalSlotNumber num:E
 * S:3:F:vol4
 *
 * Import/Export tray slots:
 * I:10:F:vol10           I:LogicalSlotNumber num:F:Volume Name
 * I:11:E              or I:LogicalSlotNumber num:E
 * I:12:F:vol40
 *
...
dlist *native_get_vol_list(UAContext *ua, STORERES *store, bool listall, bool scan)
{
   ...
   while (bnet_recv(sd) >= 0) {
      ...
      if (scan && !listall) {
          ...
          */
         vl->LogicalSlotNumber = atoi(field1);
         if (vl->LogicalSlotNumber <= 0) {
            ua->error_msg(_("Invalid LogicalSlotNumber number: %s\n"), sd->msg);
            free(vl);
            continue;
         }
         ...
         vl->PhysicalSlotNumber = INDEX_SLOT_OFFSET + vl->LogicalSlotNumber;
      } else if (!listall) {
         /*
         ...
         if (!is_an_integer(field1) || (vl->LogicalSlotNumber = atoi(field1)) <= 0) {
            ua->error_msg(_("Invalid LogicalSlotNumber number: %s\n"), field1);
            free(vl);
            continue;
         }
         ...
         /*
          * For drives the LogicalSlotNumber is the actual drive number.
          * For any other type its the actual slot number.
          */
         switch (vl->Type) {
         case slot_type_drive:
            if (!is_an_integer(field2) || (vl->LogicalSlotNumber = atoi(field2)) < 0) {
               ua->error_msg(_("Invalid Drive number: %s\n"), field2);
               free(vl);
               continue;
            }
            vl->PhysicalSlotNumber = INDEX_DRIVE_OFFSET + vl->LogicalSlotNumber;
            if (vl->PhysicalSlotNumber >= INDEX_MAX_DRIVES) {
               ua->error_msg(_("Drive number %hd greater then INDEX_MAX_DRIVES(%hd) please increase define\n"),
                             vl->LogicalSlotNumber, INDEX_MAX_DRIVES);
               free(vl);
               continue;
            }
            break;
      ...
      if (vl->VolName) {
         Dmsg6(100, "Add phys_slot = %hd logical_slot=%hd loaded=%hd type=%hd status=%hd Vol=%s to SD list.\n",
               vl->PhysicalSlotNumber, vl->LogicalSlotNumber, vl->CurrentlyLoadedSlot, vl->Type, vl->SlotStatus, NPRT(vl->VolName));
      } else {
         Dmsg5(100, "Add phys_slot = %hd logical_slot=%hd loaded=%hd type=%hd status=%hd Vol=NULL to SD list.\n",

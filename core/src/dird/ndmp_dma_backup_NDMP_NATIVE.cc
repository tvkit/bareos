...
int NdmpLoadNext(struct ndm_session *sess) {
   ...
   if ( FindNextVolumeForAppend(jcr, &mr, index, unwanted_volumes, create, prune) ) {
                                             ...
                                             struct ndm_session *sess)
{
   ...
   for (media = sess->control_acb->job.media_tab.head; media; media = media->next) {
      ...
      media->slot_addr = GetBareosSlotNumberByElementAddress(&jcr->res.wstore->rss->storage_mapping, slot_type_storage, media->slot_addr);
#if 0
   ...
   while (ndm_ee) {
      if (!jcr->db->CreateNdmpEnvironmentString(jcr, &jcr->jr,
   ...
   if (nbf_options && nbf_options->uses_level) {
      jcr->db->UpdateNdmpLevelMapping(jcr, &jcr->jr,

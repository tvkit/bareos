...
int ndmp_load_next(struct ndm_session *sess) {
   ...
   if ( find_next_volume_for_append(jcr, &mr, index, unwanted_volumes, create, prune) ) {
      ...
      slot_number_t slotnumber = get_physical_slotnumber_by_logical_slotnumber(store, slot_type_storage, mr.Slot);
      /*
                                             ...
                                             struct ndm_session *sess)
{
   ...
   for (media = sess->control_acb->job.media_tab.head; media; media = media->next) {
      ...
      media->slot_addr = get_logical_slotnumber_by_physical_slotnumber (jcr->res.wstore, slot_type_storage, media->slot_addr);
#if 0

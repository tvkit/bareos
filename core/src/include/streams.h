...
#define STREAM_UNIX_ATTRIBUTES_EX \
  5 /**< Extended Unix attr for Win32 EX - Deprecated */
#define STREAM_SPARSE_DATA 6 /**< Sparse data stream */
#define STREAM_SPARSE_GZIP_DATA \
  7                              /**< Sparse gzipped data stream - Deprecated */
...
#define STREAM_WIN32_GZIP_DATA \
  12 /**< Gzipped Win32 BackupRead data - Deprecated */
#define STREAM_MACOS_FORK_DATA 13    /**< Mac resource fork */
#define STREAM_HFSPLUS_ATTRIBUTES 14 /**< Mac OS extra attributes */
#define STREAM_UNIX_ACCESS_ACL \
  15 /**< Standard ACL attributes on UNIX - Deprecated */
#define STREAM_UNIX_DEFAULT_ACL \
  16 /**< Default ACL attributes on UNIX - Deprecated */
...
#define STREAM_ENCRYPTED_WIN32_DATA \
  21 /**< Encrypted, uncompressed Win32 BackupRead data */
#define STREAM_ENCRYPTED_SESSION_DATA \
  22 /**< Encrypted, Session Data, ASN.1, DER Encoded */
#define STREAM_ENCRYPTED_FILE_GZIP_DATA \
  23 /**< Encrypted, compressed data - Deprecated */
#define STREAM_ENCRYPTED_WIN32_GZIP_DATA \
  24 /**< Encrypted, compressed Win32 BackupRead data - Deprecated */
#define STREAM_ENCRYPTED_MACOS_FORK_DATA \
...
/**
 * Compressed streams. These streams can handle arbitrary compression algorithm
 * data as an additional header is stored at the beginning of the stream. See
 * stream_compressed_header definition for more details.
 */
...
#define STREAM_WIN32_COMPRESSED_DATA 31  /**< Compressed Win32 BackupRead data \
                                          */
#define STREAM_ENCRYPTED_FILE_COMPRESSED_DATA \
  32 /**< Encrypted, compressed data */
#define STREAM_ENCRYPTED_WIN32_COMPRESSED_DATA \
  33 /**< Encrypted, compressed Win32 BackupRead data */

#define STREAM_NDMP_SEPARATOR \
  999 /**< NDMP separator between multiple data streams of one job */

/**
 * The Stream numbers from 1000-1999 are reserved for ACL and extended attribute
 * streams. Each different platform has its own stream id(s), if a platform
 * supports multiple stream types it should supply different handlers for each
 * type it supports and this should be called from the stream dispatch function.
 * Currently in this reserved space we allocate the different acl streams from
 * 1000 on and the different extended attributes streams from 1999 down. So the
 * two naming spaces grow towards each other.
 */
#define STREAM_ACL_AIX_TEXT \
  1000 /**< AIX specific string representation from acl_get */
#define STREAM_ACL_DARWIN_ACCESS_ACL                          \
  1001 /**< Darwin (OSX) specific acl_t string representation \
        * from acl_to_text (POSIX acl)                        \
        */
#define STREAM_ACL_FREEBSD_DEFAULT_ACL                   \
  1002 /**< FreeBSD specific acl_t string representation \
        * from acl_to_text (POSIX acl) for default acls. \
        */
#define STREAM_ACL_FREEBSD_ACCESS_ACL                    \
  1003 /**< FreeBSD specific acl_t string representation \
        * from acl_to_text (POSIX acl) for access acls.  \
        */
#define STREAM_ACL_HPUX_ACL_ENTRY                         \
  1004 /**< HPUX specific acl_entry string representation \
        * from acltostr (POSIX acl)                       \
        */
#define STREAM_ACL_IRIX_DEFAULT_ACL                      \
  1005 /**< IRIX specific acl_t string representation    \
        * from acl_to_text (POSIX acl) for default acls. \
        */
#define STREAM_ACL_IRIX_ACCESS_ACL                      \
  1006 /**< IRIX specific acl_t string representation   \
        * from acl_to_text (POSIX acl) for access acls. \
        */
#define STREAM_ACL_LINUX_DEFAULT_ACL                     \
  1007 /**< Linux specific acl_t string representation   \
        * from acl_to_text (POSIX acl) for default acls. \
        */
#define STREAM_ACL_LINUX_ACCESS_ACL                     \
  1008 /**< Linux specific acl_t string representation  \
        * from acl_to_text (POSIX acl) for access acls. \
        */
#define STREAM_ACL_TRU64_DEFAULT_ACL                     \
  1009 /**< Tru64 specific acl_t string representation   \
        * from acl_to_text (POSIX acl) for default acls. \
        */
#define STREAM_ACL_TRU64_DEFAULT_DIR_ACL                 \
  1010 /**< Tru64 specific acl_t string representation   \
        * from acl_to_text (POSIX acl) for default acls. \
        */
#define STREAM_ACL_TRU64_ACCESS_ACL                     \
  1011 /**< Tru64 specific acl_t string representation  \
        * from acl_to_text (POSIX acl) for access acls. \
        */
#define STREAM_ACL_SOLARIS_ACLENT                           \
  1012 /**< Solaris specific aclent_t string representation \
        * from acltotext or acl_totext (POSIX acl)          \
        */
#define STREAM_ACL_SOLARIS_ACE                                \
  1013 /**< Solaris specific ace_t string representation from \
        * from acl_totext (NFSv4 or ZFS acl)                  \
        */
#define STREAM_ACL_AFS_TEXT \
  1014 /**< AFS specific string representation from pioctl */
#define STREAM_ACL_AIX_AIXC                         \
  1015 /**< AIX specific string representation from \
        * aclx_printStr (POSIX acl)                 \
        */
#define STREAM_ACL_AIX_NFS4                         \
  1016 /**< AIX specific string representation from \
        * aclx_printStr (NFSv4 acl)                 \
        */
#define STREAM_ACL_FREEBSD_NFS4_ACL                      \
  1017 /**< FreeBSD specific acl_t string representation \
        * from acl_to_text (NFSv4 or ZFS acl)            \
        */
#define STREAM_ACL_HURD_DEFAULT_ACL                       \
  1018 /**< GNU HURD specific acl_t string representation \
        * from acl_to_text (POSIX acl) for default acls.  \
        */
#define STREAM_ACL_HURD_ACCESS_ACL                        \
  1019 /**< GNU HURD specific acl_t string representation \
        * from acl_to_text (POSIX acl) for access acls.   \
        */
...
#define STREAM_XATTR_SOLARIS_SYS                      \
  1994 /**< Solaris specific extensible attributes or \
        * otherwise named extended system attributes. \
        */
#define STREAM_XATTR_SOLARIS 1995 /**< Solaris specific extented attributes */
#define STREAM_XATTR_DARWIN \
  1996 /**< Darwin (OSX) specific extended attributes */

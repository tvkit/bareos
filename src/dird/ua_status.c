   ...
   foreach_dlist(vl1, vol_list->contents) {
      ...
      case slot_type_drive:
         ...
         case slot_status_full:
            ua->send_msg(slot_api_drive_full_format, 'D', vl1->LogicalSlotNumber, vl1->CurrentlyLoadedSlot, vl1->VolName);
            break;
         case slot_status_empty:
            ua->send_msg(slot_api_drive_empty_format, 'D', vl1->LogicalSlotNumber);
            break;
         default:
            break;
         }
      ...
      case slot_type_import:
         ...
         case slot_status_full:
            switch (vl1->Type) {
            case slot_type_storage:
               content_send_info_api(ua, 'S', vl1->LogicalSlotNumber, vl1->VolName);
               break;
            case slot_type_import:
               content_send_info_api(ua, 'I', vl1->LogicalSlotNumber, vl1->VolName);
               break;
            default:
               break;
            }
         ...
         case slot_status_empty:
            ...
            if (vl2) {
               switch (vl1->Type) {
               case slot_type_storage:
                  content_send_info_api(ua, 'S', vl1->LogicalSlotNumber, vl2->VolName);
                  break;
               case slot_type_import:
                  content_send_info_api(ua, 'I', vl1->LogicalSlotNumber, vl2->VolName);
                  break;
               default:
                  break;
               }
            ...
            switch (vl1->Type) {
            case slot_type_storage:
               ua->send_msg(slot_api_slot_empty_format, 'S', vl1->LogicalSlotNumber);
               break;
            case slot_type_import:
               ua->send_msg(slot_api_slot_empty_format, 'I', vl1->LogicalSlotNumber);
               break;
            default:
               break;
            }
...
static void status_content_json(UAContext *ua, STORERES *store)
{
   ...
   foreach_dlist(vl1, vol_list->contents) {
      ...
      case slot_type_drive:
         ...
         ua->send->object_key_value("slotnr", vl1->LogicalSlotNumber, "%hd\n");
         switch (vl1->SlotStatus) {
         case slot_status_full:
            ua->send->object_key_value("content", "full", "%s\n");
            ua->send->object_key_value("loaded", vl1->CurrentlyLoadedSlot, "%hd\n");
      ...
      case slot_type_import:
         ...
         case slot_status_full:
            switch (vl1->Type) {
            case slot_type_storage:
               content_send_info_json(ua, "slot", vl1->LogicalSlotNumber, vl1->VolName);
               break;
            case slot_type_import:
               content_send_info_json(ua, "import_slot", vl1->LogicalSlotNumber, vl1->VolName);
               break;
            default:
               break;
            }
         ...
         case slot_status_empty:
            ...
            if (vl2) {
               switch (vl1->Type) {
               case slot_type_storage:
                  content_send_info_json(ua, "slot", vl1->LogicalSlotNumber, vl2->VolName);
                  break;
               case slot_type_import:
                  content_send_info_json(ua, "import_slot", vl1->LogicalSlotNumber, vl2->VolName);
                  break;
               default:
                  break;
               }
...
static void status_slots(UAContext *ua, STORERES *store)
{
   ...
   foreach_dlist(vl1, vol_list->contents) {
      ...
      case slot_type_storage:
      case slot_type_import:
         if (vl1->LogicalSlotNumber > max_slots) {
            ua->warning_msg(_("Slot %hd greater than max %hd ignored.\n"),
                            vl1->LogicalSlotNumber, max_slots);
            continue;
         }
         /*
          * Check if user wants us to look at this slot
          */
         if (!bit_is_set(vl1->LogicalSlotNumber - 1, slot_list)) {
         ...
         case slot_status_full:
            ...
            if (mr.VolumeName[0] && ua->db->get_media_record(ua->jcr, &mr)) {
               ...
               if (vl1->Type == slot_type_import) {
                  ...
                  ua->send_msg(slot_hformat,
                               vl1->LogicalSlotNumber,
                               ((vl1->LogicalSlotNumber == mr.Slot) ? (vl2 ? '%' : ' ') : '*'),
                               mr.VolumeName, mr.VolStatus, mr.MediaType, pr.Name);
               }
            } else {
               ua->send_msg(slot_hformat,

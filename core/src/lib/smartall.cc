 ...
 */
static struct b_queue abqueue = {&abqueue, &abqueue};
...
static void *smalloc(const char *fname, int lineno, unsigned int nbytes)
{
  ...
  if ((buf = (char *)malloc(nbytes)) != NULL) {
     ...
     */
    buf += HEAD_SIZE;
    if (++sm_buffers > sm_max_buffers) { sm_max_buffers = sm_buffers; }
    sm_bytes += nbytes;
    if (sm_bytes > sm_max_bytes) { sm_max_bytes = sm_bytes; }
    V(mutex);
  } else {
...
void sm_free(const char *file, int line, void *fp)
{
  ...
  if (cp == NULL) {
    SmartAllocMsg(__FILE__, __LINE__,
                  _("Attempt to free NULL called from %s:%d\n"), file, lineno);
  }
  ...
  if (qp->qnext->qprev != qp) {
    V(mutex);
    SmartAllocMsg(__FILE__, __LINE__,
                  _("qp->qnext->qprev != qp called from %s:%d\n"), file,
                  lineno);
  }
  if (qp->qprev->qnext != qp) {
    V(mutex);
    SmartAllocMsg(__FILE__, __LINE__,
                  _("qp->qprev->qnext != qp called from %s:%d\n"), file,
                  lineno);
  }
  ...
  if (((unsigned char *)cp)[head->ablen - 1] !=
      ((((intptr_t)cp) & 0xFF) ^ 0xC5)) {
    V(mutex);
    SmartAllocMsg(__FILE__, __LINE__,
                  _("Overrun buffer: len=%d addr=%p allocated: %s:%d called "
                    "from %s:%d\n"),
                  head->ablen, fp, get_basename(head->abfname), head->ablineno,
                  file, line);
  }
  if (sm_buffers > 0) {
    sm_buffers--;
...
void *sm_calloc(const char *fname,
                int lineno,
                unsigned int nelem,
                unsigned int elsize)
{
...
void *sm_realloc(const char *fname, int lineno, void *ptr, unsigned int size)
{
  ...
  /*  If  the  old  block  pointer  is  NULL, treat realloc() as a
     ...
     permit this.  */
  if (ptr == NULL) { return sm_malloc(fname, lineno, size); }
...
void *actuallymalloc(unsigned int size) { return malloc(size); }
...
void Actuallyfree(void *cp) { free(cp); }
...
void sm_dump(bool bufdump, bool in_use)
{
  ...
  while (ap != (struct abufhead *)&abqueue) {
    if ((ap == NULL) || (ap->abq.qnext->qprev != (struct b_queue *)ap) ||
    ...
    if (ap->abfname != NULL) {
      ...
      FPmsg6(0, "%s buffer: %s %d bytes at %p from %s:%d\n",
             in_use ? "In use" : "Orphaned", my_name, memsize, cp,
             get_basename(ap->abfname), ap->ablineno);
      if (bufdump) {
        char buf[20];
        unsigned llen = 0;
        ...
        while (memsize) {
          ...
          Bsnprintf(buf, sizeof(buf), " %02X", (*cp++) & 0xFF);
...
void sm_check(const char *fname, int lineno, bool bufdump)
{
  if (!sm_check_rtn(fname, lineno, bufdump)) {
    SmartAllocMsg(__FILE__, __LINE__,
                  _("Damaged buffer found. Called from %s:%d\n"),
...
int sm_check_rtn(const char *fname, int lineno, bool bufdump)
{
  ...
  while (ap != (struct abufhead *)&abqueue) {
    bad = 0;
    if (ap != NULL) {
      if (ap->abq.qnext->qprev != (struct b_queue *)ap) { bad = 0x1; }
      if (ap->abq.qprev->qnext != (struct b_queue *)ap) { bad |= 0x2; }
    ...
    if (bad) {
      ...
      if (bad & 0x1) { FPmsg0(0, _("  discovery of bad prev link.\n")); }
      ...
      if (ap->abfname != NULL) {
        ...
        if (bufdump) {
          ...
          while (memsize) {
            ...
            if (*cp < 0x20) {
              sprintf(errmsg + strlen(errmsg), " %02X", (*cp++) & 0xFF);
            } else {
              sprintf(errmsg + strlen(errmsg), " %c ", (*cp++) & 0xFF);
            }
            llen++;
            memsize--;
          }
...
void SmStatic(bool mode) { bufimode = mode; }
...
#ifdef xxx
void *operator new(size_t size) { return sm_malloc(__FILE__, __LINE__, size); }

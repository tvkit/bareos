...
static bool UseDeviceCmd(JobControlRecord *jcr)
{
  ...
  if (debug_level >= debuglevel) {
    foreach_alist (store, dirstore) {
      Dmsg5(debuglevel,
            "Storage=%s media_type=%s pool=%s pool_type=%s append=%d\n",
            store->name, store->media_type, store->pool_name, store->pool_type,
            store->append);
    ...
    if (me->device_reserve_by_mediatype) {
      foreach_res (rctx.device, R_DEVICE) {
        Dmsg3(debuglevel,
              ...
              rctx.device->name(), rctx.store->media_type,
...
void ReleaseReserveMessages(JobControlRecord *jcr)
{
  PopReserveMessages(jcr);
  jcr->lock();
  if (!jcr->reserve_msgs) { goto bail_out; }

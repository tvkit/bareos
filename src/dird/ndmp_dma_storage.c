      ...
      if (scan && !listall) {
         ...
         case SMC_ELEM_TYPE_DTE:
            ...
            if (edp->Full) {
               ...
               vl->SlotStatus = slot_status_full;
               slot_mapping = get_slotnumber_by_element_addr(&store->rss->storage_mapping, slot_type_storage, edp->src_se_addr);
                          ...
                          slot_number_t src_slot, slot_number_t dst_slot)
{
   ...
   slot_mapping = get_element_addr_by_slotnumber(&store->rss->storage_mapping, slot_type_storage, src_slot);
   if (slot_mapping == -1) {
   ...
   slot_mapping = get_element_addr_by_slotnumber(&store->rss->storage_mapping, slot_type_storage, dst_slot);
   if (slot_mapping == -1) {
      ua->error_msg("No slot mapping for slot %hd\n", dst_slot);
      return retval;
   }
                                       ...
                                       drive_number_t drive, slot_number_t slot)
{
   ...
   if (slot >= 0) {
       ...
       */
      slot_mapping = get_element_addr_by_slotnumber(&store->rss->storage_mapping, slot_type_storage, slot);
      if (slot_mapping == -1) {
         ua->error_msg("No slot mapping for slot %hd\n", slot);
         return retval;
      }
    ...
    * Map the logical address to a physical one.
    */
   drive_mapping = get_element_addr_by_slotnumber(&store->rss->storage_mapping, slot_type_drive, slot);
   if (drive_mapping == -1) {
      ...
      ndm_job_param& ndmp_job)
{
   ...
   driveaddress = get_element_addr_by_slotnumber(&store->rss->storage_mapping, slot_type_drive, driveindex);
   if (driveaddress == -1) {

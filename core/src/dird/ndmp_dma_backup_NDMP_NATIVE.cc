...
int NdmpLoadNext(struct ndm_session *sess)
{
  ...
  if (FindNextVolumeForAppend(jcr, &mr, index, unwanted_volumes, create,
    ...
    slot_number_t slotnumber = LookupStorageMapping(
        store, slot_type_normal, LOGICAL_TO_PHYSICAL, mr.Slot);
    /*
     * check if LookupStorageMapping was successful
     */
    if (slotnumber < 0) {
      Jmsg(jcr, M_FATAL, 0, _("LookupStorageMapping failed\n"));
      goto bail_out;
    }
    ...
    struct ndm_session *sess)
{
  ...
  for (media = sess->control_acb->job.media_tab.head; media;
    ...
    media->slot_addr =
        LookupStorageMapping(jcr->res.wstore, slot_type_normal,

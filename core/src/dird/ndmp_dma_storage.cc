...
/**
 * Generic cleanup function that can be used after a successfull or failed NDMP
 * Job ran.
 */
                         ...
                         bool scan)
{
  ...
  for (edp = smc->elem_desc; edp; edp = edp->next) {
      ...
      switch (edp->element_type_code) {
        ...
        case SMC_ELEM_TYPE_DTE:
          ...
          if (edp->Full) {
            ...
            vl->Content  = slot_content_full;
            slot_mapping = LookupStorageMapping(
                store, slot_type_normal, PHYSICAL_TO_LOGICAL, edp->src_se_addr);
     ...
     */
    vl->Slot =
        LookupStorageMapping(store, vl->Type, PHYSICAL_TO_LOGICAL, vl->Index);
                        ...
                        slot_number_t dst_slot)
{
   ...
   */
  slot_mapping = LookupStorageMapping(store, slot_type_normal,
                                      LOGICAL_TO_PHYSICAL, src_slot);
  if (slot_mapping == -1) {
                                    ...
                                    slot_number_t slot)
{
   ...
   */
  drive_mapping =
      LookupStorageMapping(store, slot_type_drive, PHYSICAL_TO_LOGICAL, slot);
  if (drive_mapping == -1) {
                          ...
                          slot_number_t slot)
{
  ...
  Dmsg4(100,
        "NdmpSendLabelRequest: VolumeName=%s MediaType=%s PoolName=%s "
        "drive=%hd\n",
        mr->VolumeName, mr->MediaType, pr->Name, drive);
  ua->WarningMsg(_("NdmpSendLabelRequest: VolumeName=%s MediaType=%s "
                   "PoolName=%s drive=%hd\n"),
                 mr->VolumeName, mr->MediaType, pr->Name, drive);

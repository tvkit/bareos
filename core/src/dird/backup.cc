...
bool DoNativeBackup(JobControlRecord *jcr)
{
   ...
   */
  if (!StartStorageDaemonJob(jcr, NULL, jcr->res.wstorage)) { return false; }
                           ...
                           const char *TermMsg)
{
  ...
  utime_t RunTime;
  MediaDbRecord mr;
  PoolMem temp, level_info, statistics, quota_info, client_options,
      daemon_status, secure_erase_status, compress_algo_list;
  ...
  bstrftimes(
      gdt, sizeof(gdt),
      jcr->res.client->GraceTime + jcr->res.client->SoftQuotaGracePeriod);
  ...
  if (jcr->ReadBytes == 0) {
    ...
    compression =
        (double)100 - 100.0 * ((double)jcr->JobBytes / (double)jcr->ReadBytes);
    if (compression < 0.5) {
      bstrncpy(compress, "None", sizeof(compress));
    } else {
  ...
  if (jcr->HasQuota) {
    ...
    Mmsg(quota_info,
         _("  Quota Used:             %s (%sB)\n"
           ...
           "  Hard Quota:             %s (%sB)\n"
           "  Grace Expiry Date:      %s\n"),
         edit_uint64_with_commas(jcr->jr.JobSumTotalBytes + jcr->SDJobBytes,
                                 ec1),
         edit_uint64_with_suffix(jcr->jr.JobSumTotalBytes + jcr->SDJobBytes,
                                 ec2),
         ...
         edit_uint64_with_suffix(jcr->res.client->HardQuota, ec8), gdt);
  }
      ...
      if (jcr->is_JobLevel(L_VIRTUAL_FULL)) {
        Mmsg(daemon_status,
             _("  SD Errors:              %d\n"
               ...
               "  Accurate:               %s\n"),
             jcr->SDErrors, sd_term_msg, jcr->accurate ? _("yes") : _("no"));
      } else {
        if (jcr->HasBase) {
          Mmsg(client_options,
               _("  Software Compression:   %s%s\n"
                 "  Base files/Used files:  %lld/%lld (%.2f%%)\n"
                 "  VSS:                    %s\n"
                 "  Encryption:             %s\n"
                 "  Accurate:               %s\n"),
               compress, compress_algo_list.c_str(), jcr->nb_base_files,
               jcr->nb_base_files_used,
               jcr->nb_base_files_used * 100.0 / jcr->nb_base_files,
               jcr->VSS ? _("yes") : _("no"), jcr->Encrypt ? _("yes") : _("no"),
          ...
          Mmsg(client_options,
               _("  Software Compression:   %s%s\n"
                 ...
                 "  Encryption:             %s\n"
                 "  Accurate:               %s\n"),
               compress, compress_algo_list.c_str(),
               jcr->VSS ? _("yes") : _("no"), jcr->Encrypt ? _("yes") : _("no"),
               jcr->accurate ? _("yes") : _("no"));
        }

        Mmsg(daemon_status,
             _("  Non-fatal FD errors:    %d\n"
               "  SD Errors:              %d\n"
               "  FD termination status:  %s\n"
               "  SD termination status:  %s\n"),
             jcr->JobErrors, jcr->SDErrors, fd_term_msg, sd_term_msg);
  ...
  Jmsg(jcr, msg_type, 0,
       ...
       jcr->jr.JobId, jcr->jr.Job, level_info.c_str(), jcr->res.client->name(),
       cr->Uname, jcr->res.fileset->name(), jcr->FSCreateTime,
       jcr->res.pool->name(), jcr->res.pool_source, jcr->res.catalog->name(),
       jcr->res.catalog_source, jcr->res.wstore->name(), jcr->res.wstore_source,
       schedt, sdt, edt, edit_utime(RunTime, elapsed, sizeof(elapsed)),
       jcr->JobPriority, statistics.c_str(), quota_info.c_str(), kbps,
       client_options.c_str(), jcr->VolumeName, jcr->VolSessionId,
       jcr->VolSessionTime, edit_uint64_with_commas(mr.VolBytes, ec7),
       edit_uint64_with_suffix(mr.VolBytes, ec8), daemon_status.c_str(),
       secure_erase_status.c_str(), TermMsg);
}
} /* namespace directordaemon */
